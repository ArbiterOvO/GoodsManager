...
import { addBrand, editBrand, getBrandById } from '@/api/brand'
// 控制抽屉显示隐藏
...
const submit = async () => {
    if (drawerStatus.value === 1) { // 新增
        const res = await addBrand(formData.value)
        if (res.data.code === 1) {
        ...
        const res = await editBrand(formData.value)
        if (res.data.code === 1) {
            visibleDrawer.value = false
...
const open = async (row) => {
    ...
    if (row.id) {
        ...
        console.log(row.id)
        const res = await getBrandById(row.id)
...
<template>
    <el-dialog v-model="visibleDrawer" :title="formData.id ? '编辑品牌' : '新增品牌'" width="500">
